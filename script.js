...
const btnCrearCurriculum = $("btnCrearCurriculum");

btnCrearCurriculum.addEventListener("click", () => {
    renderAyuda("consejosCv", "curriculum");

    $("cvNombre").value = usuario ? usuario.nombre : "";
    $("cvProfesion").value = usuario ? usuario.profesion : "";
    $("cvContacto").value = usuario
...
function generarPortafolioImpresion() {
    ...
    const tituloPortada =
        $("pfTitulo").value.trim() || "Mi portafolio";

    /*
       PORTADA
       Nombre de la obra/trabajo + datos del autor.
    */
    ...
    portada.innerHTML = `
        ...
        <h1>${escapeHtml(tituloPortada)}</h1>
        ...
        <p class="cover-bio">
            ${escapeHtml($("pfBio").value.trim() || "Presentación profesional del autor.")}
        </p>
    `;
    ...
    /*
       ...
       - rol/herramientas
       - varias imágenes
       Las imágenes se reparten automáticamente en páginas.
    */
    proyectos.forEach((proyecto, projectIndex) => {
        ...
        if (images.length === 0) {
            ...
            page.appendChild(buildProjectHeader());

            if (proyecto.enlace) {
                const link = document.createElement("p");
                link.className = "project-tools";
                link.textContent = `Proyecto: ${proyecto.enlace}`;
        ...
        /*
           4 imágenes por hoja como máximo.
           Con 1 imagen ocupa toda la fila; con 2/3/4 se ajusta.
        */
        const porPagina = 4;

        for (let offset = 0; offset < images.length; offset += porPagina) {
            const page = document.createElement("section");
            page.className = "portfolio-page portfolio-project-page";
            ...
            if (offset === 0) {
                ...
                const continuation = document.createElement("h1");
                continuation.textContent = `${projectIndex + 1}. ${proyecto.nombre} (continuación)`;
            ...
            gallery.className =
                ...
                (lote.length === 1 ? " single" : "");

            lote.forEach((src, localIndex) => {
                const wrap = document.createElement("div");
                wrap.className = "project-image-wrap";

                const img = document.createElement("img");
                img.src = src;
                img.alt = `${proyecto.nombre} - imagen ${offset + localIndex + 1}`;
            ...
            if (offset + lote.length >= images.length && proyecto.enlace) {
                const link = document.createElement("p");
                link.className = "project-tools";
                link.textContent = `Proyecto: ${proyecto.enlace}`;
                page.appendChild(link);
